...
- [ ] Tích hợp Spring Security & thư viện JWT.
- [ ] Cấu hình UserDetailsService sử dụng bảng `employees` làm tài khoản đăng nhập.
- [ ] Xây dựng luồng Đăng nhập (Login API) cấp phát JWT Token.
- [ ] Phân quyền các endpoint dựa trên vai trò nhân viên (Admin, Lễ tân, Kỹ thuật viên).
...
- [ ] Viết CRUD quản lý danh sách Nhân viên (`employees`).
- [ ] Quản lý Ca làm việc (`shifts`) và lịch trực của nhân viên (`employee_shifts`).
- [ ] API lấy danh sách Kỹ thuật viên đang rảnh trong ngày phục vụ đặt lịch.

### 📌 BƯỚC 4: Module Khách hàng & Thẻ thành viên (CRM)
*Mục tiêu: Quản lý thông tin khách hàng, hạng thẻ và lịch sử trị liệu.*
- [ ] Viết CRUD quản lý thông tin Khách hàng (`customers`).
- [ ] Quản lý hạng thành viên (`membership_tiers`) và điểm tích lũy.
- [ ] Lưu trữ và cập nhật lịch sử trị liệu (`treatment_history`) của khách.

### 📌 BƯỚC 5: Module Đặt lịch (Booking)
*Mục tiêu: Nghiệp vụ cốt lõi và phức tạp nhất của Spa.*
- [ ] API Đặt lịch hẹn (`appointments`) trực tuyến/tại quầy.
- [ ] Thuật toán kiểm tra xung đột lịch (trùng giường, trùng kỹ thuật viên).
- [ ] Gán nhiều dịch vụ (`appointment_services`) và kỹ thuật viên cho từng ca.

### 📌 BƯỚC 6: Module Kho hàng,POS & Hóa đơn (Inventory, POS & Billing)
*Mục tiêu: Quản lý nguồn tiền và trừ tồn kho tự động.*
- [ ] Tính năng Lập hóa đơn (`invoices`) & chi tiết hóa đơn (`invoice_items`).
- [ ] Tích hợp định mức tiêu hao vật tư (`service_product_usage`) tự động trừ kho (`products`) khi ca dịch vụ hoàn thành.
- [ ] Tính hoa hồng (`commissions`) tự động cho Kỹ thuật viên sau khi hóa đơn được thanh toán.

### 📌 BƯỚC 7: Module CMS & Social Omnichannel Inbox
*Mục tiêu: Hoàn thiện Landing Page và chăm sóc khách hàng tập trung.*
- [ ] API CMS: bài viết (`blog_posts`), khuyến mãi (`promotions`), banner (`banners`) để Next.js lấy dữ liệu hiển thị.
- [ ] Tích hợp Webhook kết nối MXH (Facebook, Instagram, TikTok) để thu thập tin nhắn về bảng `conversations` và `messages`.
- [ ] Kênh chat thời gian thực (Realtime Chat) qua WebSocket.
